...
public class hexgame {
	...
	final static int BORDERS = 400;
	final static int SCRSIZE = HEXSIZE * (BSIZE + 1) + BORDERS * 3; // screen size (vertical dimension).
	static int xStart = 600;
	static int yStart = 600;
	...
	void initGame() {
		...
		int screenWidth = tk1.getScreenSize().width;
		int screenHeight = tk1.getScreenSize().height;
		int xOffSet = screenWidth/4;
		int yOffSet = screenHeight/4;
		xStart = (screenWidth-3*xOffSet);
		yStart =  (screenHeight-3*yOffSet);
		...
		hexmech.setInitial(xStart, yStart);
	...
	private void createAndShowGUI() {
		...
		// width. (from h / (s+t))
		frame.setSize(tk2.getScreenSize());
	...
	class DrawingPanel extends JPanel {
		...
		class MyMouseListener extends MouseAdapter { // inner class inside DrawingPanel
			public void mouseClicked(MouseEvent e) {
				...
				/*
				 * for (int i=0;i<BSIZE;i++) { for (int j=0;j<BSIZE;j++) { board[i][j]=EMPTY; }
				 * }
				 */
